    ...
    public class SecurityLoginRepository : IDataRepository<SecurityLoginPoco>
    {
        public void Add(params SecurityLoginPoco[] items)
        {
            ...
            using (conn)
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand(@"INSERT INTO Security_Logins ([Id],[Login],[Password],[Created_Date],[Password_Update_Date],[Agreement_Accepted_Date],[Is_Locked],[Is_Inactive],[Email_Address],[Phone_Number],[Full_Name],[Force_Change_Password],[Prefferred_Language])

                                                VALUES(@Id,@Login,@Password,@Created_Date,@Password_Update_Date,@Agreement_Accepted_Date,@Is_Locked,@Is_Inactive,@Email_Address,@Phone_Number,@Full_Name,@Force_Change_Password,@Prefferred_Language)", conn);
        ...
        public IList<SecurityLoginPoco> GetAll(params Expression<Func<SecurityLoginPoco, object>>[] navigationProperties)
        {
            ...
            using (conn)
            {
                SqlCommand cmd = new SqlCommand(@"SELECT [Id], [Login], 
                                                ...
                                                [Full_Name],[Force_Change_Password],
                                                [Prefferred_Language],[Time_Stamp]
                                                FROM Security_Logins", conn);

                conn.Open();

                int x = 0;
                SqlDataReader rdr = cmd.ExecuteReader();
                SecurityLoginPoco[] SecPocos = new SecurityLoginPoco[1000];
                while (rdr.Read())
                {
                ...
                conn.Close();
                return SecPocos.Where(a => a != null).ToList();
